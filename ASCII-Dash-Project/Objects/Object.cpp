...
Object::Object(COORD coord = {0,0}, std::string ascii = "", COORD size={0,0}, int color = 0x0001, bool reverse = false) 
	: _ASCIIArt(ascii), _sizeSprite(size) , _currentColor(color), _reverseSprite(reverse)
{
	_coordObject = coord;
	Init();
}
...
void Object::ConvertASCIIArtToSpriteData()
{
	for (int i = 0; i < _sizeSprite.Y; i++)
	{
		...
		{
			if(_reverseSprite)
				_objectSprite[i][j].Char.AsciiChar = _ASCIIArt[(_sizeSprite.X * _sizeSprite.Y) - 1 - Convert2DArrayCoordTo1DArrayCoord(j, i, _sizeSprite.X)];
			else
